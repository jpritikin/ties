...
print(xtable(round(fit.df[order(fit.df$'Infit MSQ'),],2), table.placement='!h'))
@ 
...
dimnames(fit.df)[[2]] <- c("Outfit MSQ", "Outfit t", "Infit MSQ", "Infit t")
fit.crazy <- rbind(fit.df[head(order(fit.df$'Infit MSQ')),],
  fit.df[tail(order(fit.df$'Infit MSQ')),])
