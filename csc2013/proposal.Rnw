...
Karen Schmidt, Professor of Quantitative Psychology at UVa \\
Ramesh Manocha, Senior Lecturer at University of Sydney
...
and experience of mental silence.
...
Our funding needs are roughly split between these two types of evidence.
...
The pre-test will be completed by the time this proposal is reviewed.
...
a big five personality inventory \cite{goldberg1999},
and the SF36 \cite{hawthorne2007}.
...
Among our sample of 1164 respondents,
...
<<results='asis'>>=
budget <- c(500, 120 * 25, 1000 * 10)
tbl <- matrix(c(budget, sum(budget)), ncol=1)
rownames(tbl) <- c("Prepaid postage", "Research assistant, 120 hours at 25 AUD/hr", "1000 surveys at $10 per survey", "Total")
print(xtable(tbl, align=c("l","r")), include.colnames=FALSE, hline.after=c(0,nrow(tbl)-1, nrow(tbl)))
@

To run the intervention study, Dr.~Manocha will hire a research assistant at 25~AUD/hr.
Prepaid postage will cost 500~AUD.
The assistant will phone participants to remind them to complete the post-test
and perform data entry of 150-200 completed survey packets.
We anticipate that this work will take 2-3 weeks (80-120 hours).
