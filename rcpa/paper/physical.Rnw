...
conducive to flow than others. \Sexpr{nrow(demogr)}
...
The primary focus of this body of research has been on the person who experienced flow.
Hence, we regard these studies as studying \emph{personal} factors of flow.
In contrast, situational factors have received comparatively little attention.
...
running outside on a global scale of $1-100$.
However, we all know that running outside is usually
\emph{less} predictable than running on a treadmill.

Can relative comparisons be converted into absolute scores?
Fortunately, this problem is similar to
the problem of ranking a large number of Chess players.
Scoring methods are available \parencite[e.g.,][]{bradley1952}.
...
The FS2 contains 36 items, 4 items on each of 9 dimensions
derived from \textcite{csikszentmihalyi1991}.
These items concern a single person's experience.
For example, one item is, ``I know clearly what I want to do.''
The notion of flow has recently been extended to a group context \parencite{sawyer2007}.
...
this study can be regarded as exploratory.
...
Since optimal experience is not yet a widely understood notion by
the public,
detailed measures that ask about it are obliged to include
a definition \parencite[c.f.,][p.~195]{csikszentmihalyi1992}.
...
Perceived fit was regarded as measurement error.
...
and second order predictability of activities, respectively.
...
the ranking of activities by item.
By Mar 2017, a user could input her own linear combination of
items to produce a unique activity ranking.
...
the most popular response is that
...
Poorly performing items will be excluded.
We then compare a common threshold model with per-item thresholds.
Once our final model is selected then we report
...
the standard deviation $\hat\sigma$ raised to the sixth power (Equation~\ref{eqn:slope}).
This created a feedback loop to nudge $\alpha$ in the direction
that would produce more standardized item variances.
The sixth power provides a reasonable correction,
and a larger exponent creates sampler problems due to so much
...
This suggested that the parameter estimates had converged and
would not improve substantially in accuracy with additional sampling iterations.
...
we turned our attention to response curve thresholds.
...
we esimated an independence and saturated model on these same data.
...
We conducted this procedure with the factor model.
With encouraging simulation results (Figure~\ref{fig:simResults}),
we report the results obtained from real data.
...
For item validity, it is also important to check whether the
correlation structure of our retained items replicates in new samples.
...
Some activity rankings are provocative and prompt
follow up studies.
...
\emph{Sex} is another activity for which we have even less information
than \emph{martial arts} but equally burning curiosity.
...
conditions related to social flow \parencite[e.g.,][]{sawyer2007}.
In addition, 
...
Item validity is typically the primary concern with new measures.
However, at least four of our 10 items are closely modeled
on validated FS2 items (Items \emph{chatter}, \emph{present},
\emph{stakes}, and \emph{reward}).
