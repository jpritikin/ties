...
flow that incorporates activity and environmental factors.
...
During a fight scene, Jackie Chan must exercise \emph{concentration} and \emph{control}.
...
the first measures developed according to test theory procedures \parencite[e.g.,][]{crocker1986}.
...
items provide some evidence of construct validity \parencite[e.g.,][]{jackson2002,jackson2008}.
...
Balance, Concentration, Consciousness
Goal, Feedback, and Control,
Merging, Time, and Autotelic.
...
A similar FS2 item is, ``I am not concerned with what others may be thinking of me'' (Consciousness).
...
This item is more objective than FS2's subjective time distortion subscale.
...
Perhaps a correction should be made for multiple comparisons,
but this $p$-value is limited by the number of draws from the posterior
and would likely be smaller if the sampler was permitted to continue running.
...
$\log\left[ P(\sum |\hat\pi|=0) \right] < \Sexpr{log(1/numIterations)}$. This is most clearly
...
So what went wrong?
It seems likely that Items \emph{control}, \emph{spontaneous}, and
...
were presented.
The FS2 Concentration subscale was not well represented
in our item set.
...
Item \emph{complex} asked about ``action and the environment,'' which
may have been double-barreled.
...
Activities ranked with a high propensity for flow prompt follow up studies.
