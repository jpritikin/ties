...
We could only measure the demand part of the skills-demands balance.
...
The \emph{somewhat} responses were unpopular except for Item \emph{complex}.
}
...
In all items except for Item \emph{complex},
the \emph{somewhat} responses are never the most probable regardless of
difference in latent ranking.
Hence, the addition of another response category (use of a 4-point symmetric scale)
...
for $\alpha$ because we have no knowledge of
...
\begin{align}
\alpha &\equiv \left[ \frac{1}{J} \sum_{j=1}^J (1 + \lambda_j^2) \sigma_j^2 \right]^3.
\end{align}
