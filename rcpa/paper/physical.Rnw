...
commensurate with the modest amount of research that has examined
them \parencite[for a review of some work in this area, see][]{boffi2016}.
...
For example, we all know that running outside is usually
...
Due to the use of unusual models,
...
No convergence warnings were observed. All
potential scale reduction statistics $\hat R$ were well
below 1.1 \parencite[Section 11.4]{gelman2013}
which suggested that the parameter estimates had converged and
...
factor and saturated model.
Ideally, the factor and saturated model should exhibit
equally good fit.
...
correlations could improve the fit of the model.
...
than \emph{martial arts} but equal curiosity.
