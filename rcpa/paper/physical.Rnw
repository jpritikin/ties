...
A similar enthusiasm for flow induction is seen in popular books
that promise to increase the frequency and depth of flow
...
Hence, models with more parameters were favored. \Sexpr{options(digits=1)}
Note that the factor model is closer to the saturated model than the independence model
...
Hence, the factor model fit was almost as good the saturated model,
...
So what went wrong?
It seems plausible that Items \emph{control}, \emph{spontaneous}, and
...
Item \emph{complex} was a good predictor of flow even though this item
was relatively difficult to measure (Table~\ref{tab:itemInfo}).
A possibly related pattern appears in the mean posterior estimates of flow.
A natural environment seems to confer a flow propensity advantage among activities
