...
itemInfo <- data.frame(
  ...
  '$\\hat\\sigma_j$'=NA,
  '$\\max(\\hat\\theta_j)-\\min(\\hat\\theta_j)$'=NA,
  '$\\hat\\lambda_j$'=NA,
  row.names=names(sigmaByItem), check.names=FALSE)
itemInfo[rownames(loadings),c(3,4,5)] <- c(sigmaByItemFM, rangeByItemFM, loadings[,'mean'])
...
str <- kable(itemInfo,  booktabs=T, escape=FALSE, linesep = "",
  ...
  add_header_above(c(" ", "Independence Model" = 2, "Factor Model" = 3)) %>%
  footnote(escape=FALSE, threeparttable=TRUE, general_title="Note.", general=paste(
