...
\abstract{Flow training promises to increase the frequency and depth of
optimal experience.
To date, most research has concentrated on psychological,
physiological, and neuroimaging correlates of flow, all of which can
...
Statistically significant differences were found among activities,
...
ranked low and high, respectively, in flow conduciveness.
...
Combat scenes from Jackie Chan's many martial arts films (e.g., Rush Hour 1998) provide
a useful illustration of the concepts that define \emph{flow}.
Theorists have advanced a particular constellation of characteristics \parencite{engeser2012}.
During a fight, Jackie must exercise \emph{concentration} and \emph{control}.
...
\emph{enjoy} the perfect punch, block, and throw.
...
Perhaps for these same reasons,
there is growing interest among the public in flow training
\parencite[e.g.,][]{csikszentmihalyi2017,kotler2014}, a type of training that promises
to increase the frequency and depth of flow experience.
...
Prior research has examined one activity at a time or,
at most, a handful of activities.
Possible studies are constrained by available measurement instruments.
Three main approaches have been developed to study flow by self-report \parencite{moneta2012}.
At the initial stages,
in-depth qualitative studies of a few individuals were conducted \parencite[e.g.,][]{jackson1996}.
These data were used to develop a theory of flow.
A more quantitative approach was taken with the experience sampling methodology.
Participants were signaled at random times with a pager and asked to
complete an experience sampling form.
...
However, in these first two measurement approaches,
items were not amenable to the standards prescribed by test theory \parencite[e.g.,][]{crocker1986}.
The Flow State Scale-2 and Dispositional Flow Scale-2
measures were first to fill this gap \parencite[hereafter abbreviated FS2;][]{jackson2002}.
FS2 is a nine component model that has been subject to
intensive quantitative study using confirmatory factor analysis.
Although the component model exhibits acceptable psychometric properties,
it ignores the distinction between antecedents, phenomenological
indicators, and consequents of flow.

None of these measurement approaches is well suited to studying
...
how we can measure cortisol with high precision on an absolute scale from 0-30 mcg/dL.
For example, it is difficult to score the predictability of
running outside on an absolute scale of $1-100$.
Judgments that are more readily available are relative comparisons.
For example, running on a treadmill is usually
\emph{more} predictable than running outside.
...
absolute rankings \parencite[e.g.,][]{bradley1952}.
...
For example, one item is, ``I know clearly what I want to do.''
...
been described that may support group flow.
...
\subsection{Activity Flow Potential Measure}

Our goal is to develop a measure of flow for activities.
...
FS2 assesses perceived fit with items such as,
...
We could only measure the demand half of the skill-demand balance.
...
This may seem vague, but it is not a difficult criterion
...
before is novel but, perhaps, somewhat predictable.
...
more opportunities for satisfactory perceived fit.
...
A similar FS2 item is, ``I am completely focused on the task at hand.''
...
The resulting factor scores can be regarded as an activity's flow potential.
The null hypothesis is that all activities have equal flow potential.
...
Within a component, it was possible to reach from any member activity (vertex)
...
Participants completed the Activity Flow Potential Measure online.
...
Coding mostly consisted of mapping synonyms to
...
conducive they are to flow was supported. This is most clearly
...
For readers who remain concerned about our item validity,
the present study can be regarded as exploratory.
...
Activities ranked highly conducive to flow prompt follow up studies.
...
prompts further investigation due to small sample size.
By \emph{sex}, we assume that participants meant straight, heterosexual sex.
Even limited to this narrow definition,
