...
Here we investigated whether some physical activities are more
...
The primary focus was the person who experienced flow.
Hence, we regard these studies as studying personal factors of flow.
...
and many more options are available if we broaden our interest
to the general category of physical activities.
Yet, each participant may only be familiar with
...
However, we know that running outside is usually
...
This indicates that the parameter estimates have converged and
will not improve substantially in accurate with additional sampling iterations.
...
Instead of Equation~\ref{eqn:saturated}, the factor model used,
\begin{align}
\lambda &\sim \N(0, 5) \\
\pi &\sim \N(0,1) \\
...
between \Sexpr{outliers[1,'pa1']} and \Sexpr{outliers[1,'pa2']} on Item \Sexpr{outliers[1,'facet']}.
...
\begin{figure}
\begin{center}
\input{../checkSim6.tex}  % figure out better path spec TODO
\end{center}
...
found between between true and recovered item scores $\theta$ (not plotted).}
...
with $\log\left[ P(\hat\pi_\text{running} > \hat\pi_\text{martial arts})\right] < \Sexpr{log(runningVsMartialArtsP)}$.
...
differ on flow facets \parencite[e.g.,][]{wilson1987}.
...
Awareness of such situational factors may also
prompt revision to the flow metacognitions questionnaire \parencite{wilson2016}.
