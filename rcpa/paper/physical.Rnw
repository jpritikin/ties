...
These standpoints all reflect personal factors.
...
For example, $\text{flow}_\text{martial arts} - \text{flow}_\text{running}$
has a 95\% credible interval of $[\Sexpr{runningVsMartialArts[1]}, \Sexpr{runningVsMartialArts[2]}]$
with $\log\left[ P(\text{flow}_\text{running} > \text{flow}_\text{martial arts})\right] < \Sexpr{log(1/numIterations)}$. % better to compute the probability instead of assuming 1/N TODO
