...
Optimal experience is a state in which an individual is completely
immersed in an activity without reflective self-consciousness but with
a deep sense of control \parencite{csikszentmihaly1975}. % TODO better cite? is this a verbatim quote?
Optimal experience
has been vigorously studied from psychological, physiological,
and neuroimaging standpoints \parencite[personal factors; e.g.,][]{jackson1998,demanzano2010,limb2008}.
...
free-form input -- lots of missing data -- Bayesian

flow triggers, hypothesized antecedents of flow (can omit?)

Additional flow antecedents associated with a group context
have been proposed \parencite{sawyer2007}.
Items based on these triggers were not included
because they would only be applicable to some physical activities.

Complete source code for data preprocessing and analysis is
available from TODO.

\parencite{bradley1952} exploratory factor model -- novel, unaware of prior work

growth mindset applied to self control

Flow Scales-2 \parencite{jackson2002}

Survey of measurement instruments \parencite{moneta2012}
...
fit of a subject's skill to activity demands \parencite{csikszentmihaly1975,keller2012}.
...
We do not, however, assume that all activities offer
...
in Flow Scales-2 \parencite{jackson2002}.
...
factor in Flow Scales-2 \parencite{jackson2002}.
...
This item corresponds to the ``autotelic'' factor in Flow Scales-2
\parencite{jackson2002}.
...
For example, we can examine the pair \Sexpr{ppcDemoPair} on Item \emph{\Sexpr{ppcDemoFacet}},
\begin{align}
\text{observed data} &=
<<results='asis'>>=
...
However, if we require a sample size of \Sexpr{ppcSampleSizeThreshold} or more than
only \Sexpr{numPpcTests(rcd)} pairs out of \Sexpr{totalNumPairs(rcd)} are available to evaluate.
...
provide a more accurate null distribution for comparison.
...
our logistic used the difference between the latent scores
of two activities (Equation~\ref{eqn:indicator}).
...
we also esimated an independence and saturated model on the same data.
...
leave-one-out cross-validation also identified \Sexpr{nrow(outliers)} potential outliers.
However, none were concerning.
All potential outliers involved
activities with a sample size of \Sexpr{max(pmin(outliers$pa1ss, outliers$pa2ss))} or less.
...
thresholds $\tau_1, \tau_2$ were $\Sexpr{th.summary['threshold1','mean']}$,
$\Sexpr{th.summary['threshold2','mean']}$,
and $\Sexpr{alpha.summary[,'mean']}$, respectively.}
...
(Item waiting) Perhaps this is due to high flow activities like
downhill skiing that involve a lot of time waiting on the lift to get
back up the hill.

% move to present tense here and for conclusion

walking vs hiking -- Children with attention deficits concentrate better after walk in the park

running is not very conducive to flow but there is a book on it \parencite{csikszentmihalyi2017}

Only two participants compared \emph{sex} with other activities.
...
components that may support flow \parencite{daedone2011,losada2017},
would be fascinating.

Implications for flow training: You don't need to train on the facets
that you get automatically from selecting a conducive activity.  But
everybody needs to train on X, Y, Z. Talk about connection to
meditation. [TODO]

Specification of activities as free-form input
was expedient but disadvantageous in some ways.

Sample sizes between pairs of activities were
highly skewed (see Figure~\ref{fig:activity-ss}).
For example, there were 124 comparisons between \emph{running} and
\emph{weightlifting}.
A smarter data collecting website could steer participants
away from pairs of activities that already have a lot of data.
...
People with a disability that adversely affects their ability TODO

Investigate specific martial arts styles
