...
cinematic personality Jackie Chan must exercise \emph{concentration} and \emph{control}.
...
Since its formal identification \parencite{csikszentmihaly1975}, optimal experience
...
people began to think about flow in an abstract way,
separate from particular activities.
Initially researchers studied flow wherever it happened \parencite[e.g.,][Part 2]{csikszentmihalyi1988a},
...
The induction of flow is believed to involve the
...
a theory of flow was developed by the
...
However, the validity of these first two measurement instruments was
...
One way to conduct such a procedure is to
use experts to generate a candidate pool of items,
recruit subjects to respond to the candidate items,
and apply factor analysis to find
...
the first measures developed according to just such a procedure.
...
dimensions of Flow that, combined together, produce an estimate of
flow intensity or depth on a continuous scale.
Factor analyses that have examined the relationship of these
items provide further evidence of construct validity \parencite[e.g.,][]{jackson2002,jackson2008}.
...
Judgments that are more readily available are relative comparisons \parencite[e.g.,][]{jones2011,graus2015}.
For example, I experience flow \emph{more often} when I run outside
than when I run on a treadmill.
If we obtain relative comparisons, can they be converted into absolute scores?
...
absolute player rankings, even for players who did not play directly
...
parameters can be estimated even with no data.
...
A summary of our 16 items and their provenance is
...
We could only assess the demand part of the skills-demands balance.
...
Variation in perceived fit was regarded as measurement error.
...
During flow, time may seem to slow down or pass more rapidly than usual.
...
Rather than confront such vexing questions,
...
physical fitness with limited expectation of enjoyment.
...
and, that our operationalization of flow propensity reflects
...
str <- kable(cdata,  booktabs=T, escape=FALSE, linesep = "",
  ...
  footnote(escape=FALSE, threeparttable=TRUE, general_title="Note.", general=paste(
...
A standard algorithm was used to compute
all connected components \parencite{hopcroft1973}.
...
We examined the correlation structure among
the latent absolute rankings and excluded poorly performing items.
With items selected, a factor model was fit.
...
qgraph(tp, layout = "spring", graph = "cor", labels=colnames(tp),
       legend.cex = 0.3,
...
@ 
\caption{Posterior distribution of category response curves.
For a given difference in latent ranking,
the category highest in the plot is the most probable response.
The \emph{somewhat} responses were unpopular except for,
possibly, Item \emph{complex}.
}
...
we turned our attention to the item response curves
that map ordinal responses to the continuous logit scale.
All our items used a symmetric 3-point Likert scale with
response options \emph{much}, \emph{somewhat}, and \emph{equal}.
The category response curves are exhibited in Figure~\ref{fig:response-curve}.
In all items except for possibly Item \emph{complex},
the \emph{somewhat} responses were never the most probable regardless of
difference in latent ranking.
Hence, the addition of another response category (i.e., use of a 4-point symmetric scale)
is unlikely to improve measurement efficiency.
...
and saturated model \parencite{bentler1980}.
Here the independence model obtained looic \Sexpr{ind_loo$estimates['looic','Estimate']} ($SE=\Sexpr{ind_loo$estimates['looic','SE']}$),
the factor model \Sexpr{fac_loo$estimates['looic','Estimate']} ($SE=\Sexpr{fac_loo$estimates['looic','SE']}$),
and the saturated model \Sexpr{sat_loo$estimates['looic','Estimate']} ($SE=\Sexpr{sat_loo$estimates['looic','SE']}$).
...
Differences between looic statistics can also be computed.
...
For example, we compare two activities with a larger sample size at opposite ends of the scale.
...
This can be classified as a \emph{huge} effect size \parencite{sawilowsky2009}.
...
More similar activities can also be distinguished.
...
is evidence that our operationalization
...
itemInfo <- data.frame(
  '$\\hat\\sigma_j$'=sigmaByItem,
  '$\\max(\\hat\\theta_j)-\\min(\\hat\\theta_j)$'=c(rangeByItem),
  '$\\hat\\lambda_j$'=NA,
  '\\% variance$^a$'=NA,
  row.names=names(sigmaByItem), check.names=FALSE)
itemInfo[rownames(loadings), 3] <- c(loadings[,'mean'])
itemInfo[rownames(loadings), 4] <-
  apply(stdLoadings, 1, function(x) sprintf("$[%.0f, %.0f]$", 100*x[1], 100*x[2]))
...
str <- kable(itemInfo,  booktabs=T, escape=FALSE, linesep = "",
  ...
  add_header_above(c(" ", "Independence Model" = 2, "Factor Model" = 2)) %>%
  footnote(escape=FALSE, threeparttable=TRUE, general_title="Note.", general=paste(
        "Relative measurement quality is reflected by item $j$'s standard deviation $\\\\hat\\\\sigma_j$",
    ...
    alphabet=c(paste("Proportion of variance accounted for by the Flow latent factor,",
      "95\\\\% uncertainty interval.",
      "All factor model items are significantly different",
      "from zero at the $\\\\alpha=.05$ level.")))
...
in our factor model constitutes evidence that
...
So what went wrong?
When participants have trouble responding to an item they tend to
...
Items that were excluded from the factor model may reflect personal
...
in an independence model were \Sexpr{rangeByItem[,'goal1']} and
...
be distinguished from each other on these items.
...
were presented.
Item \emph{waiting} also performed poorly.
...
The FS2 Concentration subscale could have been better represented
in our item set (Table~\ref{tab:provenance}).
Item \emph{stakes} performed well,
 but simpler items might work even better.
Future studies could try items of the form,
``I concentrate much more during {{Q6}} compared to {{Q9}},''
or, ``{{Q6}} feels much more important than {{Q9}}.''

Item \emph{complex} was a good predictor of Flow
(Table~\ref{tab:itemInfo}), but a curious pattern in the mean
posterior estimates of flow $\hat\pi$ suggest that this item could be refined.
...
felt obliged to condition category specificity on
...
Raw data are made available under the Community Data License Agreement
-- Sharing -- Version 1.0.\footnote{\url{https://cdla.io/sharing-1-0/}}
...
might be reduced by the choice of
...
A more precise model of causal structure can suggest better
...
Our item model was built from the logistic function.
...
given a discrimination of $\alpha = \Sexpr{alpha.summary[,'mean']}$.
Let
\begin{align}
...
P(\text{running} < \text{running on a treadmill}) &= \frac{1}{1+\exp\left[ \alpha(\theta_1 - \theta_2) \right]} \\
 ...
 &= \frac{1}{1+\exp\left[ \Sexpr{alpha.summary[,'mean'] * (tar['predict', 'running'] - tar['predict', 'running on a treadmill'])} \right]} =  \Sexpr{1/exp(alpha.summary[,'mean'] * (tar['predict', 'running'] - tar['predict', 'running on a treadmill']))}.
\end{align}
...
was obtained for item discrimination $\alpha$; thresholds $\tau_{1j}, \tau_{2j}$;
...
Let $\tau_{1j}$ and $\tau_{2j}$ be item-specific category thresholds.
...
\tau &\sim \N(0,5) \label{eqn:thresholds} \\
...
\text{and } f(r) &\equiv \begin{cases} 
      -(\tau_{1j} + \tau_{2j}) & r=\text{much more} \\
      -\tau_{1j} & r=\text{somewhat more} \\
      0 & r=\text{equal} \\
      \tau_{1j} & r=\text{somewhat less} \\
      (\tau_{1j} + \tau_{2j}) & r=\text{much less} \\
...
Contrary to typical item response models \parencite[e.g.,][Chapter 6]{baker2004},
...
A priori, there is no appropriate stochastic distribution
for $\alpha$ because we cannot predict
...
the mean of the latent score variances raised to the third power (Equation~\ref{eqn:slope}).
...
To obtain the independence and factor models,
the correlation matrix $\Sigma$ was replaced.
The independence model used $\theta_{j} \sim \N(0, \sigma_j)$
in place of Equation~\ref{eqn:saturated}.
To obtain the factor model,
we introduced $\lambda_j$ factor loading of item $j$ and
$\pi_k$ latent flow propensity score of activity $k$
and replaced Equation~\ref{eqn:saturated} with,
\begin{align}
\lambda &\sim \N(0, 5) \\
\pi &\sim \N(0,1) \label{eqn:pi-distribution} \\
\theta_{kj} &\sim \sigma_j \cdot \N(\pi_k \lambda_j, 1).
\end{align}
The definition of $\alpha$ (Equation~\ref{eqn:slope}) also needed adjustment
because path tracing rules \parencite{boker2002} dictated a different
equation for the latent score variances,
\begin{align}
\alpha &\equiv \left[ \frac{1}{J} \sum_{j=1}^J (1 + \lambda_j^2) \sigma_j^2 \right]^3.\label{eqn:slope-fm}
\end{align}
Although Equations~\ref{eqn:slope} and \ref{eqn:slope-fm} appear
superficially different,
they both estimate the same thing and obtained
similar mean posterior estimates,
\Sexpr{alphaInd[,'mean']} and \Sexpr{alpha.summary[,'mean']}, respectively.

The posteriors were sampled using RStan~2.17.1 \parencite{rstan2.17}.
...
We considered models with thresholds set equal across items
\parencite[$\tau_{1j} = \tau_1, \tau_{2j} = \tau_2\ \forall j$; similar to][]{andrich1978}.
This constraint made little difference;
Parameter mean posteriors were correlated above .99 with
corresponding mean posteriors from the unconstrained model.
However, the constrained fit was significantly worse
when compared by looic statistics \parencite{vehtari2017}
so we chose to estimate thresholds item-wise.
...
However, the specific type of check depends on the structure of the
data.
We might compare observed and expected multinomial distributions
\parencite[similar to][]{kang2008}.
However, our multinomial distributions are specific to particular
pairs of activities.
For example, we could look at the multinomial distributions
between hiking and swimming on Item \emph{novelty}.

However, our data are very sparse from this point of view.
Textbooks often recommend that no more than 20\% of the events have
expected frequencies less than 5.
If we required a sample size of \Sexpr{ppcSampleSizeThreshold} or more
then we could only evaluate \Sexpr{numPpcTests(rcd)} pairs of activities
out of \Sexpr{totalNumPairs(rcd)}.
Since such checks would not be very informative,
