...
\keywords{flow, situational factors, factor model, pairwise comparisons,
...
flow training systems ought to recommend activities that are
particularly conducive to flow or at least account for the flow
relevant characteristics of a given specific physical activity.
}
...
Flow (or optimal) experience can be defined as a particular constellation of
characteristics that we illustrate by way of martial arts cinema.
Although many fine films have been made, here we suggest
Jackie Chan's Rush Hour 1998 as an example.
During a fight scene, Jackie must exercise \emph{concentration} and \emph{control}.
...
but a similar psychological state of flow can be found in diverse activities.
The elements that define flow include three antecedents---task demands
that slight exceed skills, clear goals, and immediate feedback---five
...
Since its formal identification in the 1970s, optimal experience
...
people began to think about inducing flow in an abstract way,
separate from particular activities.
As a research topic, we are particularly excited about flow because
consequents appear to include positive affect
and enhanced performance \parencite{landhausser2012}.
A similar enthusiasm for flow induction is seen in popular books and
courses that promise to increase the frequency and depth of flow
experience \parencite[e.g.,][]{csikszentmihalyi2017,kotler2014}.

Flow happens in the context of an activity.
...
can we provide any information to guide their choice of an activity?
Is activity choice idiographic and flow purely a matter
of fit between person and activity?
Or are there nomothetic properties of activities that
...
in-depth qualitative studies of a few individuals were conducted
to thoroughly document their experience of flow \parencite[e.g.,][]{jackson1996}.
These data were used to develop a theory of flow.
...
However, these first two measurement instruments were not designed using test theory
and have not been investigated with factor analysis
leaving some doubt about their construct validity.
Test theory concerns the valid measurement of latent constructs \parencite[e.g.,][]{crocker1986}.
The Flow State Scale-2 and Dispositional Flow Scale-2 measures were
the first measures developed according to test theory procedures
\parencite[hereafter abbreviated FS2;][]{jackson2002}.
The nine components of FS2 are balance between skills and demands (balance),
...
A shortcoming of FS2 is that it ignores the distinction between
antecedents, phenomenological indicators, and consequents of flow.
In any case, these nine components are regarded as correlated
dimensions of flow that are combined together to produce an estimate of
flow intensity or depth on a continuous scale.
Confirmatory factor analyses that have examined the relationship of these
items provide some evidence of construct validity.

Prior measurement instruments are not well suited to studying
activities apart from individuals.
Hence, prior research has examined one activity at a time or,
at most, a handful of activities.
...
assign scores to each in a way similar to
how we can measure cortisol.
...
chess players.
Fortunately, a method is available that takes a
set of chess match outcomes and determines the most likely
absolute rankings, even for players who did not play directly
against each other \parencite[e.g.,][]{bradley1952,luce1959}.
This method can be adapted to any type of relative comparison data.
...
The notion of flow has recently been extended to a group context \parencite{sawyer2007,vandenhout2016}.
A variety conditions have
been described that may support flow in a group context (``group flow'').
For example, one condition is the notion of \emph{equal participation},
that group flow will be hindered if there are
conspicuous inequalities among the group.
However, we decided not to include conditions related to group flow
because of their applicability to only a subset activities,
and we did not want to limit participants' free-form choice.

\subsection{Activity Flow Propensity Measure}

Our goal is to develop a measure of flow propensity for activities.
We use FS2, a validated measure of flow,
as a starting point and derive our items from FS2 items.
...
We will consider the nine FS2 subscales in the follow order:
\emph{balance}, \emph{concentration}, \emph{consciousness}
\emph{goal}, \emph{feedback}, and \emph{control},
\emph{merging}, \emph{time}, and \emph{autotelic}.
...
FS2 assesses perceived fit (i.e., \emph{balance}) with items such as,
...
We could only measure the demand half of the skills-demands balance.
Subjects were asked to report on voluntarily chosen activities.
We infer that perceived fit was roughly satisfactory because
the experience of anxiety or boredom is generally uncomfortable
and would not be chosen voluntarily.
...
was regarded as measurement error.

Another important antecedent of flow is the subjective value of the
activity \parencite{keller2012}.
Item \emph{stakes} drew on related concepts like, ``importance,''
and the fact that high stakes activities should recruit
more concentration than low stakes activities.
A similar FS2 item is, ``I am completely focused on the task at hand'' (\emph{concentration}).
Item \emph{evaluated} assessed the relative type of motivation,
whether extrinsic or not \parencite{csikszentmihalyi1999}.
A similar FS2 item is, ``I am not concerned with what others may be thinking of me'' (\emph{consciousness}).
...
For example, ``I know clearly what I want to do,''
``I am aware of how well I am performing,''
and ``I have a feeling of total control,'' respectively.
...
Our phenomenology items correspond with the \emph{merging}
factor in FS2.
An example of an FS2 \emph{merging} item is, ``I perform automatically,
without thinking too much.''

Although loading weakly on flow in a factor analysis \parencite{jackson2002},
...
However, we imagine activity $\times$ subjective time interactions.
...
This item is more objective than FS2's subjective time distortion.
...
This item corresponds to the \emph{autotelic} factor in FS2.
...
Our hypothesis is that physical activities differ by flow propensity.
In a cross-sectional design,
we examined many participants' experiences of activities
with respect to flow-relevant facets.
Information from these pairwise comparisons were integrated using a factor model.
The resulting factor scores $\hat\pi$ can be regarded as an activity's
absolute flow propensity on a continuous scale.
The null hypothesis is that all activities have equal flow propensity, $\sum |\hat\pi|=0$.
...
These criteria resulted in the exclusion of
...
We fit a saturated model to examine the correlation structure among
the latent absolute rankings.
Poorly performing items were excluded.
Threshold parameter $\tau_1, \tau_2$ mapped ordinal responses \emph{much}
...
Here we assumed that there was one latent factor, \emph{flow},
...
The posterior predictive check (PPC) is another way to assess model fit.
...
@ 
\caption{Posterior means and distributions of latent flow scores $\hat\pi$.
The red stripe highlights the 95\% credible interval.
...
str <- kable(itemInfo,  booktabs=T, escape=FALSE, linesep = "",
  ...
  row.names=TRUE) %>% kable_styling() %>%
    add_footnote(escape=FALSE, threeparttable=TRUE,
        ...
        'loading are cleanly separated \\parencite[e.g.,][]{kline2015}.',
...
The fits of the independence, factor, and saturated model are
exhibited in Figure~\ref{fig:model-fitness}.
The factor model fit is adequate, but there is room for improvement.
...
The \emph{somewhat} responses are never the most probable regardless of
difference in latent ranking.
Therefore, the addition of more response categories is unlikely to improve
...
Some of the effect sizes between activities are large.
For example, $\hat\pi_\text{martial arts} - \hat\pi_\text{running}$
has a 95\% credible interval of~$[\Sexpr{runningVsMartialArts[1]}, \Sexpr{runningVsMartialArts[2]}]$,
$\log\left[ P(\hat\pi_\text{running} = \hat\pi_\text{martial arts})\right] < \Sexpr{log(runningVsMartialArtsP)}$.
This $p$-value is less than the conservative Bonferroni threshold
for all pairwise comparisons of flow scores,
 $\log\left[0.05 (\frac{1}{2} \Sexpr{nrow(flow)} \cdot \Sexpr{nrow(flow)-1})^{-1}\right] =
\Sexpr{log(0.05/(nrow(flow) * (nrow(flow)-1)/2))}$.
These activities are practically at the opposite extremes, but
even similar activities can be distinguished.
For example, $\hat\pi_\text{mountain biking} - \hat\pi_\text{hiking}$
has a 95\% credible interval of $[\Sexpr{hikingVsMountainBiking[1]}, \Sexpr{hikingVsMountainBiking[2]}]$,
$P(\hat\pi_\text{hiking} = \hat\pi_\text{mountain biking}) \approx \Sexpr{hikingVsMountainBikingP}$.
Finally, item diagnostics are exhibited Table~\ref{tab:itemInfo}
wherein we find that Item \emph{complex} is a good predictor of flow
even though this item is relatively difficult to measure.

% No point in actually computing this p-value since it's even less
% than martial arts vs running.
In summary, our hypothesis that physical activities differ by how
conducive they are to flow was supported,
$\log(P(\sum |\hat\pi|=0)) < \Sexpr{log(1/numIterations)}$. This is most clearly
demonstrated by Figure~\ref{fig:activities}.
...
from running (weak propensity to flow) to martial arts (strong
propensity to flow).
...
we examined the range of latent scores $\max(\hat\theta_j)-\min(\hat\theta_j)$ obtained
...
TODO Suggest items for future studies:
FS2 \emph{concentration} -- could try ``I concentrate much more during A1 compares to A2.''
\emph{stakes} {{Q6}} feels much more important than {{Q9}}. \parencite{higgins2006}
\emph{body} Why is bodily involvement related to flow? What is the mechanism?
...
\emph{somewhat} had different meanings in different items.
...
than the independence model (Figure~\ref{fig:model-fitness}),
...
conditioned coding specificity on
...
A curious pattern appears in the mean posterior estimates of flow.
A natural environment seems to confer a flow advantage among activities
that are otherwise almost identical.
For example, 
...
theory that might explain these data \parencite{wilson1984}.
However, there are a handful of competing theories that
might account for these differences \parencite[e.g.,][]{rainisio2014}.
We suggest that cycling in an environment experimentally manipulated
using virtual reality technology may be one way to put these alternatives to the test.
...
Activities ranked with a high propensity to flow prompt follow up studies.
...
Awareness of situational factors may constitute a useful
addition to this measure.

In the Introduction, we presumed a model wherein a person
...
activities less conductive to flow like running?
More research is needed.

TODO: Prior research on flow has not really taken the activity seriously.
Suggest a re-conceptualization of flow that gives a more prominent
role to attributes of a participant's chosen activity.

Flow promises positive affect and enhanced performance.
These benefits can be enjoyed for their own sake.
However, we also look forward to a future where people enjoy flow
with vastly increased frequency and intensity.
...
rankings to extremely hazardous activities such as wingsuit flying and
breath-hold diving.
...
standard deviations $\sigma$; factor loadings $\lambda$; and latent flow scores $\pi$.
...
encouraging simulation results (Figure~\ref{fig:simResults}).
