...
Meditation can be operationalized more specifically \cite{bond2009}.
However, it is still challenging
to finding any physiological difference between meditation
and relaxation \cite{holmes1984}.
That is not to say that relaxation, or some standardized version of it,
does not offer benefits; it does \cite{benson1974}.
However, the historical context of meditation suggests that meditation
was originally conceived as much more than relaxation in
...
In terms of method, the two methods best represented in current
...
Curiously, neither mantra meditation nor mindfulness meditation value
the pivotal experience of the historical Indian yogic
tradition, that is, thoughtless awareness (a.k.a. mental silence).
...
Thoughtless awareness subsumes prescriptions
imposing limitations on permissible thought.
For example, choiceless awareness 
...
\emph{Flow} is a psychological construct that seems closely related
with thoughtless awareness.
...
Therefore, thoughtless awareness can meet the basic conditions of flow experience.

What is curious is that \citeA[p.~103]{csikszentmihalyi1991} suggested
...
What ability might yoga confer that facilitates flow?
There is little outward commonality between stereotypical yogic practices and
the diverse activities in which flow has been found.
Given that the traditional aim of yoga is thoughtless awareness,
...
limit.\footnote{Dissimilar to Heisenburg's uncertainty principle,
which places a numerical limit on the accuracy of measurement,
it seems unlikely that it will be possible to establish a
...
only turned up one article besides those by Ramesh Manocha and colleagues.
This article was concerned with mindlessness in the sense of
responding to the structure of a request rather than its content
\cite{benassi1993}.
...
In one recent review of the state of the research on meditation,
the possibility of thoughtless awareness or mental silence was not
even acknowledged \cite{shapiro2009}.
...
(\citeNP{pargament1997}; \citeNP{peterson2004}; \citeNP{snyder2009})?
...
endorsed.
However, this study did not try to discriminate
...
occurs outside the context of meditation as well, perhaps as a component of flow.
