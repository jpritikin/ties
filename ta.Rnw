...
\abstract{Complete mental silence has a rich history.}
...
<<>>=
s <- round(fivenum(fam.pcm1$etapar),2)
@ 
...
of the parameters between \Sexpr{s[2]} and \Sexpr{s[3]} and a median
parameter of \Sexpr{s[3]}.
...
<<>>=
s <- round(fivenum(fam.pcm1.p$theta.table$Person),2)
@ 
