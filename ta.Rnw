...
While the ambiguity of the English word `meditation' was identified as
early as 1977 and perhaps earlier, little has been done to resolve the
issue.
The ambiguity still remains today \cite{bond2009}, hampering meta-analysis.
Scientific studies are conducted oriented to particular
centering devices ignoring the critical question of whether participants are
experiencing complete mental silence.
...
{\dn samaadhi} (samadhi) literally means ``to acquire wholeness with.''
{\dn samaadhi} (samadhi) is a compound word consisting of three parts:
{\dn sam} (sum) means ``together'' or ``integrated,''
{\dn aa} (ah) means ``towards,''
and {\dn dhi} (dhi) means ``to hold.''
{\dn nirvicaara} (nirvichar) literally means ``without thought.''
{\dn nirvicaara} (nirvichar) is a compound word consisting of two parts:
{\dn nir} (nir) means ``no'' and {\dn vicaara} (vichar) means ``thinking.''
{\dn nirvicaara samaadhi} (nirvichar samadhi) is sometimes translated
as ``thoughtless awareness,'' however,
this phrase is somewhat misleading because thoughtlessness
can also connote carelessness.
An alternate translation is ``mental silence.''
Here ``complete mental silence'' is used to emphasize that
{\dn nirvicaara samaadhi} (nirvichar samadhi) is not merely
a reduction in the intensity of thinking but a cessation of thinking.
The cessation of thinking must be of long enough duration to disrupt
prevailing mental activity and
...
The hypothesis advanced here is that mental silence
has been overlooked.
...
``mental silence'' or ``thoughtless awareness'' \cite{fetzer1999}.
...
It seems plausible that accidental mental silence
...
that draw participants from a university subject pool (Table \ref{tab:edu}).
...
A questionnaire was developed to test these
hypotheses (Appendix \ref{appendix:survey}).
...
None of the items ask participants to attempt to express the content of
their mental silence experience.

The questionnaire can be divided into four logical sections:
basic demographic information, familiarity with mental silence (Familiarity),
context of mental silence (Context),
and the relationship between mental silence and flow (Flow).
...
fixing $\alpha=1$.
This is parameterization of GPCM
...
less than the 1.74 threshold (100 persons and 20 items, \citeNP{raiche2005})
indicating that a second dimension was unlikely.
...
Item parameters ranged from \Sexpr{s[1]} to \Sexpr{s[5]} with 50\%
of the parameters between \Sexpr{s[2]} and \Sexpr{s[4]} and
a median, mean, and standard deviation of
\Sexpr{s[3]}, \Sexpr{s[6]}, and \Sexpr{s[7]} respectively.
...
This suggests that the item may need rewording;
...
<<>>=
s <- mySummary(fam.pcm1.p$theta.table$Person)
@ 
...
@ 
\caption{Histogram of person parameters.}
...
Person parameters ranged from \Sexpr{s[1]} to \Sexpr{s[5]} with 50\%
of the parameters between \Sexpr{s[2]} and \Sexpr{s[4]} and
a median, mean, and standard deviation of
\Sexpr{s[3]}, \Sexpr{s[6]}, and \Sexpr{s[7]} respectively.
...
print(xtable(beta, digits=c(2),
             caption="Difficulty estimates for participants with and without training in meditation.",
...
Crossing thresholds suggest that there is something wrong with the survey.
Either the items need some rewording or there are too many response categories.
Item information curves are exhibited in
Figure \ref{fig:context:person:info}.
Most item difficulties are compressed near a trait score of 0.
...
<<>>=
s <- mySummary(ctx.pcm1.p$theta.table$Person)
@ 
...
Complete \texttt{R} code for the analysis presented here is available
in Appendices \ref{appendix:r-analysis} and \ref{appendix:r-presentation}.

\section{Conclusion}

Item Response Theory is a power technique for analyzing survey
response data.
