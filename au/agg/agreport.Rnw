...
# https://github.com/yihui/knitr/issues/707#issuecomment-32718629
knit_hooks$set(chunk.local = local({
  env = NULL
  function(before, options, envir) {
    knitr:::.knitEnv$knit_global = if (before) {
      env <<- knit_global()  # store the current global env, and restore later
      new.env(parent = env)  # an empty environment
    } else env
  }
}))
...
After partitioning the sample by the signs of interest in and
experience of mental silence,
there were too few samples to conduct any inferential tests (see Table~\ref{tab:by-type}).
...
Initial scores and change scores are given in Tables~\ref{tab:adhered}-\ref{tab:poorfit}
...
rownames(df) <- c('poor fit', 'humbled', 'lost interest', 'adhered')
print(xtable(df,
...
print(xtable(mk.tbl(mask), paste("Initial and change scores for the Adhered group.",
...
<<results="asis">>=
mask <- is.true(chg$msInterest < 0 & chg$msExperience >= 0)
print(xtable(mk.tbl(mask), paste("Initial and change scores for the Lost Interest group. "),
             "tab:lost"))
@

<<results="asis">>=
mask <- is.true(chg$msInterest >= 0 & chg$msExperience < 0)
print(xtable(mk.tbl(mask), paste("Initial and change scores for the Humbled group. "),
             "tab:humbled"))
@

<<results="asis">>=
mask <- is.true(chg$msInterest < 0 & chg$msExperience < 0)
print(xtable(mk.tbl(mask), paste("Initial and change scores for the Poor Fit group. "),
             "tab:poorfit"))
@
...
Categorizing participants by change is probably the wrong
way to look at the data.
For example,
a participant with high interest and high experience
may not exhibit any increase but we would still expect to see the
effect of mental silence.

% apply(chg[!success,measures], 2, mean, na.rm=TRUE)
% mention scale problem
