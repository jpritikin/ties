...
<<>>=
fitBarrer <- function(barrier.items, minItems) {
  ...
  ba <- mxModel(model="barrier", ip.mat,
                ...
                mxExpectationBA81(spec, qwidth=5, qpoints=39, minItemsPerScore=minItems),
                mxFitFunctionML(),
...
minItems <- 3
ba.est <- fitBarrer(c("msAfraid", "msIdentity", "msFast1", "msEffort", "msLife", "msEmo"), minItems)
...
ba.grp <- list(spec=ba.est$expectation$ItemSpec,
              param=ba.est$item@values,
              scores=ba.est@expectation@output$scores,
              data=espt[,colnames(ba.est$item)],
              free=ba.est$item$free,
...
fitTraining <- function(training.items, minItems) {
  ...
  train <- mxModel(model="training", imat,
                   ...
                   mxExpectationBA81(spec, minItemsPerScore=minItems, qpoints=27, qwidth=5),
                   mxFitFunctionML(),
                   mxComputeSequence(steps=list(
                     mxComputeEM('expectation', 'scores', mxComputeNewtonRaphson()),
                     mxComputeOnce('fitfunction', 'information', "meat"),
...
minItems <- 3L
tr.est <- fitTraining(c("msYearn", "msEnv", "msCause", "trainSkill", "trainNum"), minItems)
tr.grp <- list(spec=tr.est$expectation$ItemSpec,
            param=tr.est$item$values,
            data=espt[,colnames(tr.est$item)],
            minItemsPerScore=minItems,
            free=tr.est$item$free,
            qpoints=31, qwidth=5)

tr.grp$scores <- EAPscores(tr.grp)

mnf <- multinomialFit(tr.grp, omit=1)
...
<<fig.height=2>>=
got <- sumScoreEAP(tr.grp, omit=1)
df <- data.frame(score=as.numeric(rownames(got$testTbl)),
            expected=got$testTbl[,'p'] * got$n, observed=got$observed)
df <- melt(df, id="score", variable.name="source", value.name="n")
ggplot(df, aes(x=score, y=n, color=source)) + geom_line()
@
...
fitEvent <- function(event.items, minItems) {
  ...
  ev <- mxModel(model="event", ip.mat,
                ...
                mxExpectationBA81(spec, minItemsPerScore=minItems),
                mxFitFunctionML(),
                mxComputeSequence(steps=list(
                  mxComputeEM('expectation', 'scores', mxComputeNewtonRaphson()),
                  mxComputeOnce('fitfunction', 'information', "meat"),
