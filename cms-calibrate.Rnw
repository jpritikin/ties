...
<<>>=
ifaSplitBy <- function(tmpl, fullData, mMat, covMat, ...,
...
if(length(factors)==3) {
  ...
  ba <- ifaSplitBy(tmpl, ba.data, mMat, covMat, split=popSplit)
  em <- ba$compute
...
ba.data <- espt[, c('population', "msAfraid", "msIdentity", "msEmo", "msPreoccu",
...
<<fig.height=2>>=
map1 <- item.map(pop$general$barrier, 1)
...
print(plot.info(pop$general$barrier, width=5, show.total=FALSE, basis=c(1)))
@
...
if (0) {
  ...
  trM <- ifaSplitBy(train, tr.data, mMat, covMat, split=popSplit)
  em <- trM$compute
...
tr.est <- fitTraining(espt[, c('population', tr.items)])
...
<<fig.height=2>>=
toOmit <- bestToOmit(pop$general$training, 0)
...
<<fig.height=2>>=
map1 <- item.map(pop$general$training, 1)
ggplot(map1, aes(x=score, y=item, label=outcome)) + itemMapText
@
...
print(plot.info(pop$general$training, width=5, show.total=FALSE, basis=c(1)))
@
...
fitEvent <- function(ev.data) {
  ...
  ev <- ifaSplitBy(eTmpl, ev.data, mMat, covMat, split=popSplit)
  em <- ev$compute
...
<<fig.height=2>>=
toOmit <- bestToOmit(pop$general$event, 2)
...
<<fig.height=2>>=
map1 <- item.map(pop$general$event, 1)
ggplot(map1, aes(x=score, y=item, label=outcome)) + itemMapText
@
...
print(plot.info(pop$general$event, width=5, show.total=FALSE, basis=c(1)))
@

<<>>=
for (p1 in names(pop)) {
  pop[[p1]][["barrier"]]$data <- NULL
  pop[[p1]][["training"]]$data <- NULL
  pop[[p1]][["event"]]$data <- NULL
}
