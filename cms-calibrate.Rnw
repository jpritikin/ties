...
fitBarrer <- function(ba.data) {
  ...
  ip.mat$values['emo',intersect(c('wantLearn', "msAfraid", "msIdentity","msEmo","msPreoccu"),barrier.items)] <- 1
  ...
  ba <- mxModel(model="barrier", ip.mat,
                ...
                mxComputeSequence(steps=list(
                  mxComputeEM('expectation', 'scores', mxComputeNewtonRaphson(verbose=0L),
                              verbose=0L),
...
ba.data <- espt[, c("msAfraid", "msIdentity", "msFast", "msEffort", "msLife", "msEmo", "msPreoccu")]
...
<<fig.height=2>>=
baGrp1 <- omitMostMissing(ba.grp, 1)  # TODO, remove
tbl <- sumScoreEAP(baGrp1)
got <- sumScoreEAPTest(baGrp1)
...
<<results='asis'>>=
toOmit <- 1
sfit <- SitemFit(ba.grp, omit = toOmit, qpoints=31, qwidth=5)
tbl <- t(sapply(sfit, function(r) c(n=r$n, df=r$df, stat=r$statistic, pval=r$pval)))
print(xtable(tbl, paste0("Sum-score item-wise fit, omitting ",toOmit," item.")))
@
...
fitTraining <- function(training.items) {
  ...
  for (item in intersect(c('trainSkill', 'trainNum'), names(spec))) {
  ...
  imat <- mxMatrix(name="item", nrow=maxParam, ncol=numItems, free=FALSE,
                     dimnames=list(c('training', #'other', 'self',
                                     paste('p',2:maxParam,sep="")), training.items))
...
tr.est <- fitTraining(tr.items)
tr.grp <- as.IFAgroup(tr.est)
tr.grp$minItemsPerScore = 2L
...
print(plot.info(tr.grp, width=5, show.total=FALSE, basis=c(1)))
@
...
evItems <- c("msYearn", "msEnv", "msAllow",
             'freqCause', 'msTimeAlloc', 'successCat', 'maxDuration', "maxDurationOut")
ev.est <- fitEvent(evItems)

ev.grp <- as.IFAgroup(ev.est)
ev.grp$minItemsPerScore = 2L
...
<<fig.height=2>>=
ev.grp1 <- omitMostMissing(ev.grp, 2)
tbl <- sumScoreEAP(ev.grp1)
got <- sumScoreEAPTest(ev.grp1)
df <- data.frame(score=as.numeric(rownames(tbl)),
            expected=tbl[,'p'] * got$n, observed=got$observed)
df <- melt(df, id="score", variable.name="source", value.name="n")
ggplot(df, aes(x=score, y=n, color=source)) + geom_line()
...
<<results='asis'>>=
toOmit <- 2
sfit <- SitemFit(ev.grp, omit = toOmit)
tbl <- t(sapply(sfit, function(r) c(n=r$n, df=r$df, stat=r$statistic, pval=r$pval)))
print(xtable(tbl, paste("Sum-score item-wise fit, omitting", toOmit, "item.")))
@
...
print(plot.info(ev.grp, width=5, show.total=FALSE, basis=c(1)))
@
...
\includepdf[pages={3-}]{archive/short-201409.pdf}
