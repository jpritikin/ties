...
fitBarrer <- function(barrier.items) {
  ...
  ba <- mxModel(model="barrier", ip.mat,
                ...
                mxExpectationBA81(spec, qwidth=5, qpoints=39, naAction="pass"),
                mxFitFunctionML(),
...
ba.grp$score <- EAPscores(ba.grp, naAction="pass")
...
fitTraining <- function(training.items, minItems) {
  ...
  train <- mxModel(model="training", ip.mat,
                   mxData(observed=tr.data, type="raw"),
                   mxExpectationBA81(spec, naAction="pass", minItemsPerScore=minItems),
                   mxFitFunctionML(),
                   mxComputeSequence(steps=list(
                     mxComputeEM('expectation', 'scores', mxComputeNewtonRaphson()),
                     mxComputeOnce('fitfunction', 'information', "meat"),
...
minItems <- 2L
tr.est <- fitTraining(c("yearnEnv", "allowCause", "trainSkill", "trainNum"), minItems)
...
tr.grp$scores <- EAPscores(tr.grp, naAction='pass')
@
...
ev.grp <- list(spec=ev.est$expectation$ItemSpec,
            ...
            minItemsPerScore=minItems)
ev.grp$scores <- EAPscores(ev.grp, naAction = 'pass')
@ 
