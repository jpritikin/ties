...
long <- toIndividualRelativeTime(long)

wide <- ctLongToWide(long, id="id", time="time",
                     manifestNames=manifestNames, TDpredNames=c("lab"))
wide <- ctIntervalise(wide, numWaves, length(manifestNames), n.TDpred=1,manifestNames=manifestNames, TDpredNames = 'lab')

#plot means
for(i in 1:length(manifestNames)){
plot(0:8,apply(wide[,paste0(manifestNames[i],'_T',0:8)],2,mean,na.rm=T),type='b',main=paste0('mean ', manifestNames[i]))
}
