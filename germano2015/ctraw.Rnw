...
wide <- ctIntervalise(wide, numWaves, length(manifestNames), n.TDpred=1)

latents <- c("lab", "training", "ties", "envMastery")
n.TDpred <- 1
...
model <- ctModel(
  ...
  TRAITTDPREDCOV=matrix(0, nrow=length(latents), ncol=(numWaves-1)*n.TDpred),
  TRAITVAR = "auto"
  )

model$TRAITVAR[,match('lab',latents)] <- 0
model$DIFFUSION[,match('lab',latents)] <- 0
model$T0VAR[,match('lab',latents)] <- 0
model$CINT[match('lab',latents),] <- 0
model$T0MEANS[match('lab',latents),] <- 0
model$TDPREDEFFECT[match('lab',latents),] <- 0
model$DRIFT[match('lab',latents),match('lab',latents)] <- 0
# We know that training does not cause labs
model$DRIFT[match('lab',latents),match('training',latents)] <- 0

fit <- ctFit(wide, model, retryattempts=0, plotOptimization=TRUE, verbose=2, objective="mxRAM")
@
