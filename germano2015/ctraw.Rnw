...
Causally related indicators will be correlated.
However, we don't want sunrise and rooster on the same latent factor.
Hence, longitudinal factor analysis is necessary to make
sense of causally related data.
...
In addition, the life experience items load on training (\verb\msMetNum\, \verb\msSharedNum\,
\verb\msTeachNum\, and possibly \verb\msTrainTeachNum\).
This is clearly evidenced in the raw data.
...
were also assigned to training (reverse coded) because scores on these
items should move in one direction under the influence
of training. People experienced with TIES would never
...
describe circumstances that can prevent TIES even
in people with a lot of TIES experience.
...
such that all items were aligned in the same direction.
...
The chance of sensible, interpretable estimates are increased.
...
free manifestrait, will be quite small.
...
loadData <- function(approxLabTime) {
...
loadedData    <- loadData(approxLabTime=TRUE)
...
Hence, the loadings from these items to training was fixed to zero.
...
The negative loading indicates that participants with
training spent less time trying to meditate, but
spending more time did increase their experience of ties.
