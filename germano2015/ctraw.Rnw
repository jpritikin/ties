...
for (px in c(levels(partition), 'psy')) {
  ...
  if (px == 'psy') {
    m1 <- mxRename(m1, "psy")
    m1$DRIFTlog$labels[3:5,2] <- paste0("PSY", m1$DRIFTlog$labels[3:5,2])
  } else {
    m1 <- mxRename(m1, sprintf("group%02d_%02d", min(Tpoints), max(Tpoints)))
  }
  container <- mxModel(container, m1)
...
container <- mxRestore(container, strict=TRUE)
ctsummary(container$group02_05)
...
\item Examine special 1st wave items
\item IFA model for envMastery?
\end{enumerate}
