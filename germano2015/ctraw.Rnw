  ...
  if (wave == 1) {
    ...
    for (row in which(!is.na(labTime))) {
      labRow <- rbind(labRow, data.frame(
        id=raw[row, 'id'], time=as.numeric(labGrid[labTime[row],])/7))
    }
...
toIndividualRelativeTime <- function(df, id="id", time="time") {
  idList <- unique(df[[id]])
  for (ix in idList) {
    mask <- df[[id]] == ix
    t0 <- min(df[mask,time])
    df[mask,time] <- df[mask,time] - t0
  }
  df
}
...
# long <- toIndividualRelativeTime(long) #unnecessary and problematic

#generate group vector based on observations
long<-long[order(long[,'id'],long[,'time']),]
id<-long[,'id']
numObsUnique<- unlist(lapply(1:length(unique(id)), function(x){
  length(id[id==unique(id)[x]])
  }))
numObs<-c()
for(i in 1:length(id)){
numObs[i]<-numObsUnique[id[i]==unique(id)]
}

long<-cbind(long,numObs)
  
wide <- ctLongToWide(long, id="id", time="time",
                     manifestNames=manifestNames, TDpredNames=c(TDpredNames))
numWaves<-23
wide <- ctIntervalise(wide, numWaves, length(manifestNames), n.TDpred=1,manifestNames=manifestNames, TDpredNames = TDpredNames)

wide[,paste0(TDpredNames,'_T',0:(numWaves-2))][is.na(wide[,paste0(TDpredNames,'_T',0:(numWaves-2))])] <- 0
...
model <- ctModel(
  ...
  LAMBDA=lambda, manifestNames=manifestNames, latentNames=latents, TDpredNames = "lab",
  T0TDPREDCOV=matrix(0, nrow=length(latents), ncol=(numWaves-1)*n.TDpred),
  TRAITTDPREDCOV=matrix(0, nrow=length(latents), ncol=(numWaves-1)*n.TDpred), #I don't think we know this is 0 - unless the lab or control condition was randomly assigned?
  TDPREDMEANS=matrix(apply(wide[,paste0(TDpredNames,'_T',0:(numWaves-2))],2,mean,na.rm=T),ncol=1),
...
model$CINT[match('labeffect',latents),] <- 0
model$CINT[,] <- 0
model$MANIFESTMEANS <- matrix(paste0('mean_',manifestNames),ncol=1)
model$T0MEANS[match('labeffect',latents),] <- 0
model$T0MEANS[,] <- 0
# model$TDPREDEFFECT[match('lab',latents),] <- 0 #this is the wrong way around
...
fit <- ctFit(wide, model, retryattempts=10, verbose=2,meanIntervals=T,stationary=c('T0VAR','T0MEANS'))
