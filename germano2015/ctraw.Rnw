...
for (Tpoints in sort(unique(numUniqueObs))) {
  if (Tpoints < 3) next
  long1 <- subset(long, id %in% longUniqueIds[which(numUniqueObs == Tpoints)])
  if (Tpoints == 3) {
    # combine 1,2,3 because ctsem can't handle Tpoints=c(1,2) right now
    long1 <- rbind(long1, subset(long, id %in% longUniqueIds[which(numUniqueObs == 1)]))
    long1 <- rbind(long1, subset(long, id %in% longUniqueIds[which(numUniqueObs == 2)]))
  }
  print(c(Tpoints,nrow(long1)))
  wide <- ctLongToWide(long1, id="id", time="time",
                       manifestNames=manifestNames, TDpredNames=TDpredNames)
  wide <- ctIntervalise(wide, Tpoints, n.manifest=length(manifestNames), n.TDpred=n.TDpred,
                        manifestNames=manifestNames, TDpredNames = TDpredNames)
  ...
  model <- ctModel(
    n.manifest=length(manifestNames), n.latent=length(latents), Tpoints=Tpoints, n.TDpred=n.TDpred,
    LAMBDA=lambda, manifestNames=manifestNames, latentNames=latents, TDpredNames=TDpredNames,
    T0TDPREDCOV=matrix(0, nrow=length(latents), ncol=(Tpoints-1)*n.TDpred),
    #  TRAITTDPREDCOV=matrix(0, nrow=length(latents), ncol=(Tpoints-1)*n.TDpred), #I don't think we know this is 0 - unless the lab or control condition was randomly assigned?
    TDPREDMEANS=matrix(apply(wide[,paste0(TDpredNames,'_T',0:(Tpoints-2)), drop=FALSE],2,mean,na.rm=T),ncol=1),
  ...
  m1 <- ctFit(wide, model, nofit=TRUE, stationary=c('T0VAR','T0MEANS'))
  m1 <- mxRename(m1, paste0("group", Tpoints))
