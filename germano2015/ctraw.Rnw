...
latents <- c("training", "ties", "envMastery")
...
model <- ctModel(
  n.manifest=length(manifestNames), n.latent=length(latents), Tpoints=numWaves, n.TDpred=1,
  LAMBDA=lambda, manifestNames=manifestNames, latentNames=latents, TDpredNames = "lab")
fit <- ctFit(wide, model, retryattempts=0, plotOptimization=TRUE, verbose=2)
@
