...
for (px in levels(partition)) {
  ...
  # replace NAs in the TDpredNames columns with zeros
  # This should be unneccesary, I think
  for (tx in 0:(max(Tpoints)-2)) {
    col <- paste0(TDpredNames, "_T", tx)
    wide[is.na(wide[,col]), col] <- 0
  }
