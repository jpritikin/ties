...
#  factors <- c('training', 'fear')
  ...
  imat <- set.nominal.rank(spec, imat, 'msMetNum', 0, 0.7)
  ...
  if (any('backward' == factors)) {
    imat$values['backward', intersect(c('msEffort', 'msIdentity'), training.items)] <- 1
    imat$free['backward', intersect(c('msEffort', 'msIdentity'), training.items)] <- TRUE
  }
...
tr.items <- c('msNotion', 'msEffort', 'msIdentity', 'freqCause',
              'msTimeAlloc',
